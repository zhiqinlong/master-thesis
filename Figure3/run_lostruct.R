...
for (k in seq_along(bcf.files)) {
    ...
    for (chrom.index in seq_along(file.positions)) {
        ...
        if (file.exists(pca.file)) { 
            ...
            system.time( 
                        pca.stuff <- eigen_windows( win.fn, k=opt$npc, w=opt$weights, mc.cores=120 ) 
                    )
...
load('pc.distmat.RData')
#system.time( pc.distmat <- pc_dist( all.pcas, npc=opt$npc ,mc.cores=125) )
#dim(pc.distmat)
#save(pc.distmat, file = "pc.distmat.RData")
#class(pc.distmat)
# MDS on the resulting distance matrix
mds.file <- file.path( opt$outdir, "mds_coords_20.csv" )
...
save(all.lengths,chroms,seq_len,all.pcas,opt, file = "temp.RData")
